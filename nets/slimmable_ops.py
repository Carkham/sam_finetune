...
def convert_us_module(module, us=[True, True], max_width=1.0):
    r"""Helper function to convert all :attr:`Conv2D` and :attr:`Linear` layers in the model to
    :class:`USConv2d` and :class:`USLinear` layers.

    Args:
        module (nn.Module): module containing one or more :attr:`BatchNorm*D` layers

    Returns:
        The original :attr:`module` with the converted :class:`USConv2d` and :class:`USLinear`
        layers. If the original :attr:`module` is a :attr:`Conv2D` or :attr:`Linear` layer,
        a new :class:`USConv2d` and :class:`USLinear` layer object will be returned
        instead.
    """
    module_output = module
    if isinstance(module, nn.Conv2d):
        module_output = USConv2d(
            module.in_channels,
            module.out_channels,
            module.kernel_size,
...
class USLayerNorm(nn.LayerNorm):
    def __init__(self, num_features, ratio=1, width_max=1.0):
        num_features_max = int(make_divisible(
            num_features * width_max / ratio) * ratio)
        super(USLayerNorm, self).__init__(num_features_max)
        self.num_features_basic = num_features
        # for tracking log during training
        self.width_max = width_max
        self.ratio = ratio
    ...
    def forward(self, input):
        ...
        c = int(make_divisible(
            self.num_features_basic * self.width_mult / self.ratio) * self.ratio)
        return nn.functional.layer_norm(input, (c,), weight[:c], bias[:c], self.eps)
